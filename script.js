...
function adCard(data, key) {
    ...
    <div class="cardstyling col-lg-4 col-sm-6 portfolio-item">
      ...
      <p class="validate card-text">${data.description.substring(4,data.description.length)}</p>
      
      <h5>${data.price} Etb</h5>
      <button type="button" class="btn btn-warning" onclick="signInFirst()">Add To Cart</button>
        </div>
      </div>
    </div>
  `
  ...
  <div class="cardstyling col-lg-4 col-sm-6 portfolio-item">
    <div class="card h-100">
      ...
      <p class="validate card-text">${data.description.substring(4,data.description.length)}</p>
      <h5>${data.price} Etb</h5>
      <button type="button" class="btn btn-warning" onclick="addToFavourites(this)">Add To Cart</button>
    </div>
    </div>
  </div>
`
...
const adsRef = database.ref("ads").orderByChild("price");
console.log(adsRef)

function fetchAds() {

    adsRef.on('child_added', function(data) {
        if (data.val().description.substring(0, 4) === "abel") {
...
function addToFavourites(button) {
    ...
    var price = button.previousElementSibling.innerHTML;
